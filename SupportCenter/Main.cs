...
namespace SupportCenter
{
    public partial class frmMain : Form
    {
        ...
        private void clientToolStripMenuItem1_Click(object sender, EventArgs e) // Show Client
        {
            ...
            if (mctlClient == null || this.pnlControlPanel.Controls.Contains(mctlClient) == false)         
            {
        ...
        {
            // create new Client control if necessary
            if (mctlItem == null || this.pnlControlPanel.Controls.Contains(mctlItem) == false)
            {
        ...
        private void frmMain_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Shift && e.KeyCode ==  Keys.A)            // Shift-A: something
            {
                e.SuppressKeyPress = true;  
            }
            else if (e.Shift && e.KeyCode == Keys.C)            // Shift-C: Open/Show Client screen
            {
                e.SuppressKeyPress = true;
                clientToolStripMenuItem1_Click(this, new EventArgs());
            }
            else if (e.Shift && e.KeyCode == Keys.T)            // Shift-T: Timesheet screen
            {
                e.SuppressKeyPress = true;
                timesheetToolStripMenuItem_Click(this, new EventArgs());
            }
            else if (e.Shift && e.KeyCode == Keys.D)            // Shift-D: Item Database screen
            {
                e.SuppressKeyPress = true;
                itemToolStripMenuItem1_Click(this, new EventArgs());
            }
            else if (e.Shift && e.KeyCode == Keys.M)        // Shift-M: Messages
            {
                e.SuppressKeyPress = true;
                tsddNewMessage_Click(this.tsddNewMessage, new EventArgs());
            }
            else if (e.Shift && e.KeyCode == Keys.S)        // Shift-S: Save
            {
                e.SuppressKeyPress = true;
            }
            else if (e.Shift && e.KeyCode == Keys.W)        // Shift-W: Close Top Control in Control Panel
            {
